...
def trapezoid_compound_rule(func, a, b, n):
    ...
    integral = h * (0.5 * (y_points[0] + y_points[-1]) + np.sum(y_points[1:-1]))
...
def generate_trapezoid_graph(func, a, b, subintervals):
    ...
    try:
        ...
        for i, n in enumerate(subintervals):
            ...
            title = f"Regla del Trapecio Compuesto con {n} subintervalo{'s' if n > 1 else ''}, ∫({a},{b}) = {integral_value:.6f}"
            if has_exact:
...
def generate_interactive_trapezoid_graph(func, a, b, max_subintervals=10):
    ...
    try:
        ...
        title = f"Regla del Trapecio Compuesto para ∫({a},{b}) {func} dx"
        if has_exact:
...
def generate_romberg_method_interactive_plot(func, a, b, romberg_table):
    ...
    x_points = np.linspace(a, b, n + 1)
    y_points = f(x_points)
    fig.add_trace(go.Scatter(x=x_points, y=y_points, mode="markers", name="Puntos de evaluación", marker=dict(color="red")))
    fig.update_layout(
...
def calculate_romberg_method(request):
    try:
        ...
        romberg_table = romberg_method_integration(func, a, b, max_level)
        ...
        plot_json = generate_romberg_method_interactive_plot(func, a, b, romberg_table)
        try:
